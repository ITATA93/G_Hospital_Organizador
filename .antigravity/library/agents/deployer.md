...
1. **NUNCA** incluir secretos en archivos de configuración
2. **SIEMPRE** usar variables de entorno para credenciales
3. **SIEMPRE** usar imágenes con tags específicos (no `latest`)
...
CODEX_MODEL_REASONING_EFFORT=high codex exec \
  ...
  "Genera Dockerfile para aplicación Python FastAPI.
...
CODEX_MODEL_REASONING_EFFORT=xhigh codex exec \
  --dangerously-bypass-approvals-and-sandbox \
  "Genera manifests de Kubernetes para la aplicación:
