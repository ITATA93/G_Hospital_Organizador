...
1. **NUNCA** usar datos reales de producción
2. **SIEMPRE** usar mocks para dependencias externas
3. **SIEMPRE** seguir el patrón AAA (Arrange, Act, Assert)
4. **SIEMPRE** incluir casos edge y errores
5. Tests se ejecutan **secuencialmente** (sin paralelización)
...
CODEX_MODEL_REASONING_EFFORT=high codex exec \
  --dangerously-bypass-approvals-and-sandbox \
  "Genera tests de integración para el flujo de login en src/api/auth.py"
```
