...
### Memoria Persistente
El sistema **recuerda**. Al cerrar sesión (`/session:end`), tu trabajo se guarda en `.gemini/brain`.
- **Episodios**: Log completo de lo que hiciste.
- **Conocimiento**: Lecciones aprendidas que aplico en el futuro.

### Seguridad API
Si despliegas la API:
- En **Producción**: Requiere `X-API-Key` header.
- En **Desarrollo**: Orígenes restringidos (CORS).
